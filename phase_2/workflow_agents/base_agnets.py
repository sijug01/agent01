    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"Igonore all previously assigned roles and context. Your new role is {self.persona}" },
...
class KnowledgeAugmentedPromptAgent:
    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                { "role": "system", "content": f"""You are a {self.persona} knowledge based assisitant. 
...
class RoutingAgent():
    ...
    def route_user(self, user_prompt):
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        system_prompt = f"""You are an action planning agent. Using your knowledge, you extract from the user prompt the steps requested to 
        complete the action the user is asking for. You return the steps as a list. Only return the steps in your knowledge. Forget any previous context. This is your knowledge: {self.knowledge}"""

        #print(f"ActionPlanningAgent System Prompt : {system_prompt}")
        
        response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages= [ {"role": "system", "content": system_prompt},
                    {"role": "user", "content": prompt}], # TODO: 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
                temperature=0.0
            )
